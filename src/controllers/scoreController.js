...
export default class ScoreController extends Phaser.GameObjects.Text {
    ...
    start() {
        if (this.orderDisplay === null) throw "ScoreController class: orderDisplay not yet attached";
        if (this.assemblyTables.length === 0) throw "ScoreController class: no assemblyTable attached yet";
        this.started = true;
    }
    ...
    submit(index) {
        ...
        for (var i in this.assemblyTables) this.assemblyTables[i].setRequiredItems(this.requiredItems);
